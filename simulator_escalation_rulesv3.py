...
def determine_highest_level(row, thresholds):
    levels = [
        ('DS', get_level_value(determine_ds_level(row['DS'], thresholds))),
        ('GM', get_level_value(determine_gm_level(row['GM'], thresholds))),
        ('VOL', get_level_value(determine_vol_level(row['Vol'], thresholds))),
        ('OBR', get_level_value(row['oBR_level']))
    ]
...
def display_threshold_controls(thresholds, country):
    ...
    new_vol_l4 = st.sidebar.number_input('VOL L4 Threshold', value=float(thresholds['vol_l4']), key=f'vol_l4_{country}')
    new_vol_l3 = st.sidebar.number_input('VOL L3 Threshold', value=float(thresholds['vol_l3']), key=f'vol_l3_{country}')
    new_vol_l2 = st.sidebar.number_input('VOL L2 Threshold', value=float(thresholds['vol_l2']), key=f'vol_l2_{country}')
    new_vol_l1 = st.sidebar.number_input('VOL L1 Threshold', value=float(thresholds['vol_l1']), key=f'vol_l1_{country}')
    
    st.sidebar.subheader('GM Thresholds')
    new_gm_l4 = st.sidebar.slider('GM L4 Threshold', 0, 100, int(thresholds['gm_l4']), key=f'gm_l4_{country}')
    new_gm_l3 = st.sidebar.slider('GM L3 Threshold', 0, 100, int(thresholds['gm_l3']), key=f'gm_l3_{country}')
    new_gm_l2 = st.sidebar.slider('GM L2 Threshold', 0, 100, int(thresholds['gm_l2']), key=f'gm_l2_{country}')
    new_gm_l1 = st.sidebar.slider('GM L1 Threshold', 0, 100, int(thresholds['gm_l1']), key=f'gm_l1_{country}')
    
    st.sidebar.subheader('DS Thresholds')
    new_ds_l5 = st.sidebar.slider('DS L5 Threshold', 0, 100, int(thresholds['ds_l5']), key=f'ds_l5_{country}')
    new_ds_l4 = st.sidebar.slider('DS L4 Threshold', 0, 100, int(thresholds['ds_l4']), key=f'ds_l4_{country}')
    new_ds_l3 = st.sidebar.slider('DS L3 Threshold', 0, 100, int(thresholds['ds_l3']), key=f'ds_l3_{country}')
    new_ds_l2 = st.sidebar.slider('DS L2 Threshold', 0, 100, int(thresholds['ds_l2']), key=f'ds_l2_{country}')
    new_ds_l1 = st.sidebar.slider('DS L1 Threshold', 0, 100, int(thresholds['ds_l1']), key=f'ds_l1_{country}')
    
    new_thresholds = {
        'vol_l4': new_vol_l4, 'vol_l3': new_vol_l3, 'vol_l2': new_vol_l2, 'vol_l1': new_vol_l1,
        'gm_l4': new_gm_l4, 'gm_l3': new_gm_l3, 'gm_l2': new_gm_l2, 'gm_l1': new_gm_l1,
        'ds_l5': new_ds_l5, 'ds_l4': new_ds_l4, 'ds_l3': new_ds_l3, 'ds_l2': new_ds_l2, 'ds_l1': new_ds_l1
    }
